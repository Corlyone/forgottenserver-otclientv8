...
void ProtocolSpectator::release()
{
	...
	if (player && casterProtocol && player->client == casterProtocol->shared_from_this()) {
		...
		TextMessage message = TextMessage(MESSAGE_STATUS_CONSOLE_RED, name + " has left the channel.");
...
void ProtocolSpectator::syncChannels()
{
	...
	for (const auto channel : channels) {
		...
		if (channelUsers.find(player->getID()) != channelUsers.end()) {
			sendChannel(channel->getId(), channel->getName());
		}
	}
	...
	casterProtocol->sendChannelEvent(CHANNEL_CAST, name, CHANNELEVENT_JOIN);
}
